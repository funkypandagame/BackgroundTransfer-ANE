...
public class DownloadRequestQueue {
    ...
    List<DownloadRequest> getAll() {
        List<DownloadRequest> toReturn;
		synchronized (mCurrentRequests) {
            toReturn = new ArrayList<DownloadRequest>(mCurrentRequests.size());
			for (DownloadRequest request : mCurrentRequests) {
    ...
    int cancel(String downloadURL) {
        synchronized (mCurrentRequests) {
            for (DownloadRequest request : mCurrentRequests) {
                if (request.getUri().toString().equals( downloadURL ) ) {
                    request.cancel();
                    return 1;
                }
            }
